...
use druid::{BoxConstraints, Data, Env, Event, EventCtx, LayoutCtx, Lens, LifeCycle, LifeCycleCtx, PaintCtx, Point, Size, TimerToken, UpdateCtx, Widget, kurbo::Ellipse, piet::Text, widget::ListIter};
use druid::kurbo::{Rect, Line};
use druid::piet::Color;
use druid::piet::kurbo::Shape;
use druid::RenderContext;
use satellite_data::{database::Database, satellites::{MajorBody, Satellite}};
use evalexpr::*;
...
impl ListIter <SatelliteWrapper> for SatelliteteVec {
    ...
    fn for_each_mut(&mut self, mut cb: impl FnMut(&mut SatelliteWrapper, usize)) {
        for satellite in self.0.iter_mut() {
            cb (satellite, satellite.clone().satellite)
        }
    }
...
impl Widget <AstronomyCanvasData> for AstronomyCanvas {
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, _data: &AstronomyCanvasData, _env: &Env) -> Size {
        let default_size = Size::new(ctx.window().get_size().width, ctx.window().get_size().height);

        bc.constrain(default_size)
    
    }

    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut AstronomyCanvasData, _env: &Env) {
        match event {
            ...
            Event::MouseUp(_mouse_event) => {
                ...
                let satellites: Vec<SatelliteWrapper> = self.selected_satellites.clone().into_iter().map(|satellite| SatelliteWrapper {
                    satellite: satellite.id, selected: false,
                } ).collect();
        ...
        data.selected_satellite = self.selected_satellite.clone();
    }
    fn update(&mut self, ctx: &mut UpdateCtx, old_data: &AstronomyCanvasData, data: &AstronomyCanvasData, _env: &Env) {
        if old_data.selected_satellites.0.len() == data.selected_satellites.0.len() && data.selected_satellites.0.len() > 0 {
            let satellite: Vec<SatelliteWrapper> = data.selected_satellites
                .0
                .clone()
                .iter()
                .enumerate()
                ...
                .map(|(_id, satellite)| satellite.clone())
                .collect();
            if satellite.len() > 0 {
                self.selected_satellite = Some(satellite.first().unwrap().clone());
                ctx.request_timer(Duration::from_millis(1));
            }
        }
        if data.toggle_distance != old_data.toggle_distance || data.toggle_angle != old_data.toggle_angle || data.toggle_major_semiaxes != old_data.toggle_major_semiaxes {
            ctx.request_paint();
            ctx.request_layout();
        }
        if old_data.center != data.center {
            if self.move_bul {
                self.center = Point::new(self.center.x + old_data.center.x - data.center.x, self.center.y + old_data.center.y - data.center.y);
            }
            ctx.request_paint();
            ctx.request_layout();
        }
        if data.match_string != old_data.match_string {
            ...
            let database: Vec<Satellite> = database
                .data
                .iter()
                .map(|satellite: &Satellite| satellite.clone() )
                .filter(|satellite: &Satellite| parse_logicall_expression(satellite, &data.match_string))
                .collect();
    ...
    fn lifecycle(&mut self, _ctx: &mut LifeCycleCtx, _event: &LifeCycle, _data: &AstronomyCanvasData, _env: &Env) {
...
impl AstronomyCanvas {
    fn render_circular_view(&mut self, ctx: &mut PaintCtx, data: &AstronomyCanvasData, _env: &Env) {
        ...
        for satellite in &self.database.data {
            let a: f64 = satellite.orbital_params.major_semiaxis.into();
            let e: f64 = satellite.orbital_params.eccentricity.into();
            ...
            let rotation: f64 = satellite.orbital_params.inclination.into();
            ...
            let x1 = self.center.x + (200. * rotation.cos());
            let y1 = self.center.y + (200. * rotation.sin());  
            
            // let x2 = self.center.x - (200. * rotation.cos());
            // let y2 = self.center.y - (200. * rotation.sin());  

            let line = Line::new((self.center.x, self.center.y), (x1, y1));
            ...
            match data.mouse_point {
                Some(mouse_point) => {
                    ...
                    for ell in ell {
                        if ell.intersect_line(diagonal).len() > 0 {
                            if !self.selected_satellites.contains(&satellite) {
            ...
            if data.toggle_major_semiaxes {
                ...
                let rotation: f64 = satellite.orbital_params.ascending_node.into();
        ...
        if data.toggle_distance {
            ...
            for i in 0..50 {
                ...
                let line = Line::new((self.center.x+distance, self.center.y-15.0), (self.center.x+distance, self.center.y+15.0));
                let text = ctx.text();
                let text= text.new_text_layout((distance * scale).to_string()).text_color(Color::WHITE);
        ...
        if data.toggle_angle {
            ...
            for i in 0..36 {
                ...
                let text = ctx.text();
                let text= text.new_text_layout((i * 10).to_string()).text_color(Color::WHITE);
                let text = text.build().unwrap();
    ...
    fn render_graph_view(&mut self, ctx: &mut PaintCtx, data: &AstronomyCanvasData, _env: &Env) {
        let x_values: Vec<&str> = data.x_value.split("|").collect();
        let x_values: Vec<&str> = x_values.into_iter().filter(|&value| !value.is_empty()&&value!=" ").collect();
        let mut x_scale = 1.0;
        ...
        x_scale = x_scale * data.scale;

        let y_values: Vec<&str> = data.y_value.split("|").collect();
        let mut y_value = String::new();
        let y_values: Vec<&str> = y_values.into_iter().filter(|&value| !value.is_empty()&&value!=" ").collect();
        let mut y_scale = 1.0;
        if y_values.len() > 0 {
        ...
        y_scale = y_scale * data.scale;

        let x_line = Line::new((self.center.x-1000.0, self.center.y), (self.center.x+1000.0, self.center.y));
        let y_line = Line::new((self.center.x, self.center.y-1000.0), (self.center.x, self.center.y+1000.0));
        ctx.stroke(x_line, &Color::WHITE, 2.0);
        ctx.stroke(y_line, &Color::WHITE, 2.0);
        for i in -50..50 {
            let distance = i as f64 *80.0;
            let x_line = Line::new((self.center.x+distance, self.center.y+10.0), (self.center.x+distance, self.center.y-10.0));
            let y_line = Line::new((self.center.x+10.0, self.center.y+distance), (self.center.x-10.0, self.center.y+distance));
            ctx.stroke(x_line, &Color::WHITE, 1.0);
            ctx.stroke(y_line, &Color::WHITE, 1.0);
            let text = ctx.text();
            let text_x= text.new_text_layout((distance * x_scale).to_string()).text_color(Color::WHITE);
            let text_x = text_x.build().unwrap();
            let text_y= text.new_text_layout((distance * y_scale).to_string()).text_color(Color::WHITE);
        ...
        for satellite in &self.database.data {
            ...
            match data.mouse_point {
                Some(mouse_point) => {
                    ...
                    // ctx.stroke(diagonal, &Color::AQUA, 1.0);    
                    // println!("{},{},{},{}",rect.x0, rect.x1, rect.y0, rect.y1);
                    if x < rect.x1 && x > rect.x0 && y < rect.y1 && y > rect.y0{
                        if !self.selected_satellites.contains(&satellite) {
                            self.selected_satellites.push(satellite.clone());
                        }
                    }
...
fn parse_logicall_expression(satellite: &Satellite, logical_expression: &String) -> bool {
...
fn parse_math_expression(satellite: &Satellite, math_expression: &String) -> f64 {
